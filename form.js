...
// v 0.0.1
window.FormFast = function (settings) {
    settings = $.extend(true, {
        checkSettings: {
            action: {
                final: function (done, value) {
                    done();return
                }
            }
        }
...
FormFast.prototype.check = function (settings) {
    var self = this
    settings = $.extend(true, self.checkSettings, settings)
    // jquery对象转化为 dom
    if (typeof settings.ref.get === 'function') {
        settings.ref = settings.ref.get(0)
    }
    for(var when in settings.action) {
        if (when !== 'final') {
            // 防止闭包影响 when 取值
            (function(when) {
                $(settings.ref).on(when, function () {
                    settings.action[when](function (message) {
                        self.__check(message, [], settings, when)
                    }, settings.ref.value)
                })
            })(when)
        }
    }
    if (self.checks[settings.id]) {
        throw new Error('form.check(settings) 存在重复 id: ' + settings.id)
    }
    self.checks[settings.id] = settings
}
FormFast.prototype.__check = function (message, errors, check, when) {
    var self = this
    if (message) {
        var errorInfo = {
            message: message,
        ...
        self.settings.onError(errorInfo.message, errorInfo.check, when)
        errors.push(errorInfo)
    }
    else {
        self.settings.onPass(check)
    }
}
FormFast.prototype.checkAll = function (checkAllCallback) {
    var self = this
    var key
    var eachItem
    var promises = []
    ...
    for(key in self.checks) {
        eachItem = self.checks[key]
        var checkRule = eachItem.action.final || eachItem.action.blur
        if (typeof checkRule === 'undefined') {
            throw new Error('action 必须存在  final 或 blur')
        }
        rules.push({
            rule: checkRule,
            check: eachItem
    ...
    $.each(rules, function (index, item) {
        item.rule(function (message) {
            checkEndCount++
            var when = ':checkAll'
            self.__check(message, errors, item.check, when)
            if (checkEndCount == rules.length) {
                if (errors.length === 0) {
